...
# Script to update your stake pool registration without paying the deposit again (for updating your pledge or other details)

if [ "$#" -ne 1 ]; then
    echo "Usage: build-update-stake-pool-registration-transaction.sh TTL"
    exit 2
fi
...
remaining=$(expr $amountInAddress - $minFee)
